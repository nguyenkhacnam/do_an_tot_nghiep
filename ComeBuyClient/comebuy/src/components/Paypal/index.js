...
import { Dialog, Button, Backdrop, CircularProgress } from "@mui/material";
      ...
      for (let j = 0; j < prodList.length; j++) {
        if (cartList[i].productid === prodList[j].productID) {
          ...
          stringOrder =
            stringOrder +
            "\n" +
            `${prodList[j].name} - Số lượng: ${
              cartList[i].amount
            } - Giá tiền: ${item.total.toLocaleString("en-US")} VND`;
        }
      }
    }
    if (localStorage.getItem("role") === "customer") {
      emailApi
        .sendOrder({
          ...
          text:
            "Cảm ơn bạn đã đặt hàng trên trang ComeBuy. \n" +
            "Đơn hàng của bạn: \n" +

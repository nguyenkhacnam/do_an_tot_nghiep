...
exports.createInvoice = async (req, res) => {
  ...
  try {
    if (
      ...
      !req.body.userID ||
      !req.body.address ||
      req.body?.listGoods?.length === 0
    ) {
      console.log("undefined");
      res.status(400).send({
        message:
          "Invoice controller >> create invoice failed >> lack of information!",
      });
      return;
    }
    ...
    listProds?.map(pro1 => {
      remain.map(pro2 => {
        if (
          pro2.productid === pro1.productid &&
          pro2.remaining < pro1.quantity
        ) {
    ...
    if (resultFilterNonRemain.length != 0) {
      ...
      return res.status(400).send({
        status: "error",
        message:
          "There is product that does not have enough quantity to supply",
        data: resultFilterNonRemain,
      });
    } else if (!createInvoice) {
      await t.rollback(transaction.data);
      return next(
        new AppError("Invoice controller >> create invoice failed "),
        404
      );
    } else {
...
exports.createInvoiceItem = async (req, res) => {
  ...
  try {
    ...
    //     }
    // }, { transaction: transaction.data });
    const createInvoiceItem = await InvoiceItem.create(invoiceItem, {
      transaction: transaction.data,
    });
    ...
    res.status(500).send({
      message:
        error.message || "Some error occurred while creating the Invoice item.",
    });
  }
};
    ...
    const allInvoice = await Invoice.findAll(
      {
        include: [
          ...
          },
          {
            ...
            include: [
              // Include Product model through InvoiceItem
              {
                model: Product,
                as: "product",
...
exports.updateInvoice = async (req, res) => {
  ...
  // console.log("🚀 ~ file: invoice.controller.js:238 ~ exports.updateInvoice= ~ transaction:", req.body)
  try {
    if (!transaction.status && transaction.error) {
      throw transaction.error;
    }
    ...
    if (isPaid) {
      ...
      const foundInvoice = allInvoice.find(
        invoice => invoice.invoiceID === invoiceID
      );

      if (foundInvoice) {
        foundInvoice.invoiceitem.forEach(async item => {
          ...
          try {
            ...
            if (foundStock) {
              ...
              const newRemaining = foundStock.remaining - amount;

              // Cập nhật lại giá trị remaining trong Stock
              await Stock.update(
              ...
              console.log(
                `Updated remaining for product ${productid} is ${newRemaining}`
              );
            } else {
              console.log(`Stock not found for product ${productid}`);
            }
          } catch (error) {
    ...
    res.status(500).send({
      message: err.message || "Some error occurred while update invoice by id.",
    });
  }
};
